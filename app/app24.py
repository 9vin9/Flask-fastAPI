...
login_manager = LoginManager(app)   # 애플리케이션에 LoginManager 적용
login_manager.login_view = 'login'  # 로그인 페이지의 뷰 함수 이름을 설정

# 사용자 모델 정의
class User(UserMixin, db.Model):
    id = db.Column(db.Integer, primary_key=True)    # 사용자의 ID, 고유식별자
...
def protected():
    return f'Logged in as {current_user.username}'  # 현재 로그인한 사용자 이름 표시
